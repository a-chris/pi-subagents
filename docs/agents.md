...
---
name: my-cli
description: One-shot analysis through a local CLI
runner:
  type: external-cli
  command: my-cli
  args: ["--print"]
...
| `defaultContext: fork` | Prefer forked session context for this agent. The call cannot override it — mode is agent-owned (M2). If the parent has no persisted session file or current leaf yet, the implicit default falls back to `fresh` without a failed first attempt. Explicit launch `context` is internal/extension-only and `context: "fork"` remains strict when set. |
| `defaultContext: summary` | Prefer a role-directed context brief distilled from the parent session for this agent. The brief is prepended to the child task (alongside a launch `prequel` when one is supplied; the prequel is consumed only for `fork`/`summary` modes and stays absent for `fresh`). Generation failures (or a missing persisted parent session) fall back to `fresh` with a warning and never abort the run. |
| `contextBrief` | Optional role-directed instruction shaping the agent's `summary` context brief (for example a scout wants entry points and open questions, a reviewer wants decisions and diffs). Absent, a generic distillation instruction is used. Declaring it does not enable summary mode — the mode comes only from the agent's own `defaultContext`. |
...
| `defaultContext` | Optional `fresh`, `fork`, or `summary` launch-context preference for this agent. A launch that omits `context` resolves from it (missing → `fresh`); the call cannot override the agent's mode. Implicit `fork`/`summary` fall back to `fresh` when the parent has no persisted session file or current leaf; explicit launch `context: "fork"`/`context: "summary"` (internal/extension-only) remains strict. |
