...
Public seams for other Pi extensions and host integrations: the in-process RPC, the structured delegation API, launch preflight, capability ceilings, and the background-work provider contract.
...
- `managementActions` — the exact management actions accepted by RPC `manage`.
...
- Preflight reads the extension config, so launch defaults follow each agent's declared `defaultContext`; explicit `context` still wins.
...
Public status exposes bounded audit counts and sources, never full extension paths.
