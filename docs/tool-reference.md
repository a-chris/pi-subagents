...
| `context` | `fresh \| fork \| summary \| profile` | global or per-agent default, else `fresh` | Explicit `fresh`, `fork`, or `summary` overrides every workflow child. `profile` requires the selected agent's declared `defaultContext` and ignores config `defaultSubagentContext`; missing agent defaults fail. When omitted, [`defaultSubagentContext`](configuration.md#defaultsubagentcontext) wins over each agent's `defaultContext`; implicit fork/summary fall back to fresh without a persisted parent session and leaf. Explicit fork is strict. `summary` distills the parent session into a role-directed brief (see [`contextBrief`](agents.md)) prepended to the child task; generation failures fall back to fresh. Packaged `worker`, `oracle`, and `advisor` default to `fork`. |
