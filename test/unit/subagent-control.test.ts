...
describe("subagent control attention state", () => {
	...
	it("marks non-exempt open tools for attention at the active threshold", () => {
		...
		assert.equal(shouldEmitOpenToolAttention({ config: defaults, currentTool: "bg_wait", currentToolStartedAt: 0, now: 999_999 }), false);
	...
	it("dedupes notifications once per child target and attention state", () => {
		...
		assert.match(controlNotificationKey(event), /^run-1:0:needs_attention:idle:[a-f0-9]{8}$/);
