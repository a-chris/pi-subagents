...
describe("profiles helpers", () => {
	...
	it("keeps machine placement when a model profile replaces the override map", () => {
		...
		fs.writeFileSync(path.join(profilesDir, "quota.json"), JSON.stringify({
			subagents: { agentOverrides: { scout: { model: "openai-codex/gpt-5.3-codex-spark" }, reviewer: { thinking: false } } },
		}, null, 2));
		const settingsPath = path.join(homeDir, ".pi", "agent", "settings.json");
		fs.mkdirSync(path.dirname(settingsPath), { recursive: true });
		fs.writeFileSync(settingsPath, JSON.stringify({
			subagents: { agentOverrides: { reviewer: { machine: "workmac", model: "old" }, worker: { machine: "workmac" }, stale: { model: "remove-me" } } },
		}, null, 2));
		...
		assert.deepEqual(written.subagents.agentOverrides, {
			...
			reviewer: { thinking: false, machine: "workmac" },
