 ...
 setTimeout(() => process.exit(90), 15000).unref();
			...
			try {
				...
				if (mode !== "abort") {
					...
					mockPi.onCall({ steps: [{ jsonl: [events.toolStart("bg_wait", { reason: "need_decision", message: "Ready" })] }, { waitForPath: releaseChild, jsonl: [events.assistantMessage("child A done")] }] });
					child = executor.execute("lifecycle-A", { async: false, agent: "worker", task: "A", worktree: true, acceptance: false }, controller.signal, (update) => {
						if (update.details?.progress?.some((entry) => entry.currentTool === "bg_wait")) childReady();
	...
	it("delivers delegated activity-state transitions despite heartbeat suppression", async () => {
		...
		mockPi.onCall({
			steps: [
				{ jsonl: [events.toolStart("bg_wait", { message: "waiting" })] },
				{ waitForPath: activeReleasePath, jsonl: [events.toolEnd("bg_wait"), events.toolResult("bg_wait", "done")] },
