...
- **Trimmed missions to `mission.create`.** The mission patch actions (`mission.list`, `mission.show`, `mission.update`, `mission.resolve-decision`, `mission.attach-run`, `mission.close`) and the mission patch-metadata params (`missionId`, `missionUpdate`, `missionStatus`, `missionScope`, plus the attach-only `runMode`/`runStatus` and close-only `summary`) were removed from the tool surface. Missions keep durable records, automatic mission creation for workflows, the durable `state.get/set` workflow global, and `mission.create`; attaching a workflow to an existing mission is an internal launch-contract capability (`missionId` on the executor params) only. Goal pause/resume/opt-out is an operator edit to the mission record (`goal.status`) instead of a `mission.update` action.

- **Removed the watchdog subsystem.** The `watchdog.*` control actions (`watchdog.status`, `watchdog.check`, `watchdog.configure`, `watchdog.recommend-model`), the `src/watchdog/` engine (runtime, review, LSP pre-pass, child status, launch rules, permission arbiter, settings) and the `subagents.watchdog.*` settings keys were deleted, along with the `/subagents-watchdog` slash command, the child watchdog status events and tail-timer gating, the acceptance `watchdog-blocker` runtime check, the notify/TUI/Fleet watchdog displays, and `docs/watchdog.md`. The watchdog was a redundant second path over config and composed review; the reviewer workflow and normal composition cover the same job. Watchdog scoring-related params `scope`/`target`/`thinking` and the `focus` inspector param teardown: `scope`/`target`/`thinking` were watchdog-only and are removed; `focus` remains an inspector param (M4).

- **Removed durable schedules.** The `schedule.*` control actions (`schedule.create`, `schedule.list`, `schedule.show`, `schedule.history`, `schedule.pause`, `schedule.resume`, `schedule.run`, `schedule.run-due`, `schedule.delete`), the `src/runs/background/scheduled-runs.ts` engine and its `scheduledRuns`/`schedules` config and result surfaces, and the schedule-launch trigger params (`at`, `every`, `timezone`, `catchUp`, `overlap`, `on`, `name`, `sessionOnly`, `quiet`) were removed. Durable schedules were removed because cron/OS scheduling is a first-class equivalent; `scheduleOrigin` tracking, the `scheduled` mission run mode, and RPC/slash/notify schedule displays were removed with them.

- **Removed multi-lane orchestration.** The `runs.lanes(...)` workflow DSL (lane/stage batches, lane plan messages and graph projection), the per-child `lane` launch metadata (`WorkflowLaneMetadata`) across types, schemas, executor, async status/receipts, and the TUI widget, and the `lane.*` control actions (`lane.status`, `lane.recordMerge`, `lane.recordSupersession`) were deleted. The `usageBudget` param (root-only reported usage), the top-level `planId`/`laneId`/`merge`/`supersession` surface params, merge/supersession evidence writing and cleanup-eligibility evaluation on worktree handoff manifests, and the `skills/pi-subagents/references/multi-lane-orchestration.md` reference were also removed. The preflight `lanes` plan contract (display-only planned lanes) is unchanged.

- **Added `context: "summary"` subagent launch mode.** A role-directed context brief is distilled from the parent session (user text, assistant text, and tool-call names, newest-first; thinking and tool-result bodies skipped; no compaction-summary reuse) and prepended to the child's task via the existing fork-preamble channel. The optional `contextBrief` agent frontmatter field shapes the brief per work type (scout wants entry points, reviewer wants decisions and diffs); without one a generic distillation instruction is used. Opt-in per launch (`context: "summary"`), per agent (`defaultContext: summary`), or via config (`defaultSubagentContext: "summary"` with optional `summaryContext` model/size settings); implicit resolution requires a persisted parent session, explicit stays strict. Any generation failure falls back to a fresh launch with a warning and never aborts the run.

- **Removed the Herdr saved-machine feature and the remaining supervisor channel.** The Herdr integration is gone: `herdr-connection`, `herdr-placed-run`, `herdr-pi-protocol`, `herdr-machine`, the `herdr-pi-bridge` remote-child mode, the `herdr-status` bridge, and the `src/inspectors/herdr` project-pane plugin were deleted, along with the `src/api/project-panes` surface and the `project.open`/`project.status`/`project.close` actions. The `machine` option (frontmatter, settings overrides, launch param, chain/parallel steps), saved-machine placement, and remote Git evidence were removed end-to-end; Herdr saved machines are no longer accepted anywhere. All remaining `contact_supervisor`/`subagent_supervisor` and intercom references were swept from runtime code, builtin agent prompts, docs, and tests; children that need a parent decision now return `BLOCKED: <reason>` as a terminal result. `bg_wait` remains the only timeout-exempt coordination tool.
