...
function parseExternalCliReceiptMetadata(value: unknown, key: string, source: string): ExternalCliReceiptMetadata | undefined {
	...
	if (metadata.safety !== undefined) throw new Error(`${label}.safety is invalid for the generic adapter.`);
...
function parseEntry(value: unknown, key: string, source: string): WorkflowReceiptEntry {
	...
	return { ...(value as WorkflowReceiptEntry), ...(terminalOutcome ? { terminalOutcome } : {}), ...(acceptanceRecovery ? { acceptanceRecovery } : {}) };
}
