...
import { discardPreservedWorktrees, formatParallelHandoffError, formatParallelHandoffReference, parallelHandoffPath, writeParallelHandoffGroup, writeWorktreeSetupHandoff } from "../shared/parallel-handoff.ts";
...
function childTerminalStatus(input: { exitCode?: number | null; interrupted?: boolean; detached?: boolean; stopped?: boolean; timedOut?: boolean; turnBudgetExceeded?: boolean; processSignal?: string | null; success?: boolean; state?: string; acceptanceRejected?: boolean }): "completed" | "failed" | "paused" | "stopped" | "detached" | "blocked" {
	if (input.detached === true) return "detached";
	if (input.stopped === true || input.processSignal) return "stopped";
	if (input.timedOut === true || input.turnBudgetExceeded === true) return "failed";
	if (input.interrupted === true) return "paused";
	if (input.state === "paused" || input.state === "stopped") return input.state;
	const ok = input.success === true || (input.success === undefined && input.acceptanceRejected !== true && (input.exitCode === 0 || input.state === "completed" || input.state === "complete"));
...
function createForegroundControlNotifier(data: Pick<ExecutionContextData, "controlConfig" | "contextPolicy" | "params">, deps: Pick<ExecutorDeps, "pi" | "state">): (event: ControlEvent) => void {
	return (event) => {
...
async function runSinglePath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	try {
		const launched = await runSync(ctx.cwd, agents, params.agent!, task, compactOptional<Parameters<typeof runSync>[4]>({
			...
			onDetachedExit: async (result) => {
				...
				try {
					if (worktreeSetup) {
						await finalizeSingleWorktreeHandoff({ worktreeSetup, artifactsDir, runId, cwd: sourceCwd, agent: params.agent!, result, workflowKey: params.workflowKey });
					}
					try {
	...
	if (worktreeSetup) {
		worktreeHandoff = r.detached
			...
			: await finalizeSingleWorktreeHandoff({ worktreeSetup, artifactsDir, runId, cwd: sourceCwd, agent: params.agent!, result: r, workflowKey: params.workflowKey });
	}
...
function workflowChildResult(
	...
	const terminalOutcome = forcedTerminalOutcome
		?? (result.details.results.some((child) => child.timedOut)
			...
			: result.details.results.some((child) => child.turnBudgetExceeded || child.toolBudgetBlocked)
							...
							onChildSettled: (notification) => {
								...
								try {
									deps.pi.sendMessage(
										...
										{ triggerTurn: incrementalChildCompletionTriggersTurn(notification) },
									);
								} catch (sendError) {
