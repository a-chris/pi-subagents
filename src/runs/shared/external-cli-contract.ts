...
export function parseExternalCliCapabilityNarrowing(value: unknown, label: string): ExternalCliCapabilityNarrowing | undefined {
	...
	for (const [key, setting] of Object.entries(input)) {
		if (setting !== false) throw new Error(`${label}.${key} may only be false; the generic external-cli runner already caps these capabilities.`);
	}
...
export function normalizeExternalCliRunnerStatus(value: unknown): ExternalCliRunnerStatus | undefined {
	...
	// SAFETY: This function is the parser boundary for runner status persisted in
	// run artifacts and receipts; `unknown` input is narrowed field by field before use.
	const input = value as Record<string, unknown>;
...
export function externalCliReceiptMetadata(input: {
	...
	if (input.externalProcess) {
		...
		if (input.outputReference || input.externalProcess.finalOutputPath) {
			metadata.outputArtifacts.finalOutputPath = input.outputReference ?? input.externalProcess.finalOutputPath;
		}
	} else if (input.outputReference) {
		metadata.outputArtifacts = { finalOutputPath: input.outputReference };
	}
	return metadata;
}